...
def _bars_per_day_from_index(idx) -> float:
    ...
    return max(1.0, 1440.0 / minutes)  # 일중 바 수

def volatility_filter(df: pd.DataFrame, min_vol: float = 0.015, max_vol: float = 0.08) -> bool:
    """적정 변동성 범위 체크(주기 자동 추정)"""
    returns = df['close'].pct_change().dropna()
    if len(returns) < 50:
        return False

    bpd = _bars_per_day_from_index(df.index)
    vol = returns.tail(50).std() * np.sqrt(bpd)  # 일중 표준화
...
STRATEGY_MAP: Dict[int, Callable] = {
    ...
    2: strat_conservative_breakout,  # ✅ 활성화: 보수적 브레이크아웃
    # 3: strat_weak_down_swing,
    4: strat_defensive_trend_follow,  # ✅ 활성화: 중립~상승 대응
    5: strat_neutral_box_scalp,  # ✅ 활성화: 중립 시장 스캘핑
...
def decide_order(df: pd.DataFrame, balance_krw: float = 1_000_000, 
    ...
    # MSI 상단일수록 기대수익 하한 완화(진입 허들 낮춤)
    if plan and getattr(plan, "min_expected_pct", None) is not None:
        msi = calc_market_index(snap)
        if msi >= 7:
            # 예: 0.17% -> 0.12% 수준
            plan.min_expected_pct = max(0.0010, plan.min_expected_pct * 0.7)
