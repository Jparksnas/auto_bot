...
class Config:
    ...
    MIN_ORDER_AMOUNT: float = float(os.getenv('MIN_ORDER_AMOUNT', '5000'))
    
    # 리스크 관리
    MAX_POSITION_PCT: float = float(os.getenv('MAX_POSITION_PCT', '0.10'))  # 20% → 10%
    DAILY_LOSS_LIMIT_PCT: float = float(os.getenv('DAILY_LOSS_LIMIT_PCT', '0.03'))  # 5% → 3%
    MAX_TRADE_RISK_PCT: float = float(os.getenv('MAX_TRADE_RISK_PCT', '0.02')) 
    
    # ===== 추가: 손절/익절 기본값 =====
    SL_PCT_DEFAULT: float = float(os.getenv('SL_PCT_DEFAULT', '0.015'))  # 기본 손절 -1.5%
    TP_PCT_DEFAULT: float = float(os.getenv('TP_PCT_DEFAULT', '0.030'))  # 기본 익절 +3%
    MAX_HOLD_SEC: int = int(os.getenv('MAX_HOLD_SEC', '3600'))  # 최대 보유 시간 1시간
    ...
    # === 체결 보호 / 슬리피지 가드 ===
    USE_LIMIT_PROTECT: bool = os.getenv('USE_LIMIT_PROTECT', 'true').lower() == 'true'
    MAX_SLIP_BPS_SELL: int = int(os.getenv('MAX_SLIP_BPS_SELL', '5'))
    MAX_SLIP_BPS_BUY:  int = int(os.getenv('MAX_SLIP_BPS_BUY',  '5'))
    LIMIT_IOC_TIMEOUT: float = float(os.getenv('LIMIT_IOC_TIMEOUT', '1.2'))  # 기본 1.2s로 타이트
    # 신규: 사다리/폴백 상한을 .env로 노출 (main의 기본값과 맞춤)
    BUY_RETRY_LADDER_BPS_STR: str = os.getenv('BUY_RETRY_LADDER_BPS', '0,3,6,9')
    MARKET_FALLBACK_BPS_CAP: int  = int(os.getenv('MARKET_FALLBACK_BPS_CAP', '9'))
    POS_COOLDOWN_AFTER_IOC_SEC: float = float(os.getenv('POS_COOLDOWN_AFTER_IOC_SEC', '1.5'))
    TP_DEBOUNCE_SEC: float = float(os.getenv('TP_DEBOUNCE_SEC', '3.0'))
    
    FAST_SPIKE_MULT: float = float(os.getenv("FAST_SPIKE_MULT", "2.7"))
    MIN_EXPECTED_PCT: float = float(os.getenv("MIN_EXPECTED_PCT", "0.0012"))
    SLIPPAGE_TOLERANCE_PCT: float = float(os.getenv("SLIPPAGE_TOLERANCE_PCT", "0.0012"))
    ...
    def validate_config(cls) -> tuple[bool, list[str]]:
        ...
        # === 슬리피지 가드 범위 검증 ===
        if not (0 <= cls.MAX_SLIP_BPS_SELL <= 50):
            errors.append("MAX_SLIP_BPS_SELL는 0~50 bps 범위여야 합니다 (기본 5)")
        if not (0 <= cls.MAX_SLIP_BPS_BUY <= 50):
            errors.append("MAX_SLIP_BPS_BUY는 0~50 bps 범위여야 합니다 (기본 5)")
        if not (0.5 <= cls.LIMIT_IOC_TIMEOUT <= 10.0):
            errors.append("LIMIT_IOC_TIMEOUT은 0.5~10.0초 범위여야 합니다")
    ...
    def print_config(cls):
        ...
        print()
        print("[Execution Guard / Slippage]")
        print(f"Limit Protect:     {cls.USE_LIMIT_PROTECT}")
        print(f"Max Slip Sell:     {cls.MAX_SLIP_BPS_SELL} bps (~{cls.MAX_SLIP_BPS_SELL/100:.2f}%)")
        print(f"Max Slip Buy:      {cls.MAX_SLIP_BPS_BUY} bps (~{cls.MAX_SLIP_BPS_BUY/100:.2f}%)")
        print(f"IOC Timeout:       {cls.LIMIT_IOC_TIMEOUT:.2f}s")
...
def create_env_from_legacy():
    ...
    # 체결 보호 / 슬리피지 가드 기본값
    env_content.append("# Execution Guard / Slippage")
    env_content.append("USE_LIMIT_PROTECT=true")
    env_content.append("MAX_SLIP_BPS_SELL=5")
    env_content.append("MAX_SLIP_BPS_BUY=5")
    env_content.append("LIMIT_IOC_TIMEOUT=2.5")
    ...
    USE_LIMIT_PROTECT: bool = os.getenv('USE_LIMIT_PROTECT', 'true').lower() == 'true'
        ...
        # === 체결 보호 / 슬리피지 가드 ===
    USE_LIMIT_PROTECT: bool = os.getenv('USE_LIMIT_PROTECT', 'true').lower() == 'true'
    MAX_SLIP_BPS_SELL: int = int(os.getenv('MAX_SLIP_BPS_SELL', '5'))  # 매도 슬리피지 제한 (bps 단위)
    MAX_SLIP_BPS_BUY: int  = int(os.getenv('MAX_SLIP_BPS_BUY', '5'))   # 매수 슬리피지 제한 (bps 단위)
    LIMIT_IOC_TIMEOUT: float = float(os.getenv('LIMIT_IOC_TIMEOUT', '2.5'))  # IOC 주문 타임아웃
    POS_COOLDOWN_AFTER_IOC_SEC: float = float(os.getenv('POS_COOLDOWN_AFTER_IOC_SEC', '1.5'))
    TP_DEBOUNCE_SEC: float = float(os.getenv('TP_DEBOUNCE_SEC', '3.0'))
    ...
    def get(cls, key, default=None):
        """
        config_loader가 없을 때를 위한 호환 get()
        - 1순위: 클래스에 정의된 속성 (예: MIN_ORDER_AMOUNT 등)
        - 2순위: 환경변수
        - 3순위: 기본값(default)
        """
        if hasattr(cls, key):
